...
def inject_motion_embeddings(model, sizes=[320, 640, 1280], modules=['up','down','mid']):
    ...
    for name, module in model.named_modules():
        if 'motion_modules' in name and re.search(r'transformer_blocks\.\d+$', name):
            replacement_dict[f'{name}.pos_embed'] = MotionEmbedding(embed_dim=module.norms[0].normalized_shape[0]).to(dtype=model.dtype, device=model.device)
